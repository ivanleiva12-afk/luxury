...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Generate unique ID como STRING para DynamoDB
    const registroId = String(Date.now());
  ...
  function setupAutoUsername() {
    ...
    displayNameInput.addEventListener('input', () => {
      ...
      // Update placeholder with live suggestion (versión rápida sin sufijo)
      if (!usernameInput.value.trim() && displayValue.length >= 2) {
        let suggested = displayValue.toLowerCase().trim()
    ...
    displayNameInput.addEventListener('blur', () => {
      ...
      if (!usernameInput.value.trim() && displayValue.length >= 2) {
        // Generar username INSTANTÁNEAMENTE sin consultar base de datos
  ...
  function generateUsernameInstant(displayName) {
    ...
    // Agregar sufijo único con timestamp corto para garantizar unicidad
    const uniqueSuffix = Date.now().toString().slice(-4); // Últimos 4 dígitos del timestamp
