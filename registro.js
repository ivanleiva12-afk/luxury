...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function saveRegistration(registro) {
    try {
      ...
      const existingRegistros = JSON.parse(localStorage.getItem('pendingRegistros') || '[]');
      existingRegistros.push(registro);
      
      // Intentar guardar
      localStorage.setItem('pendingRegistros', JSON.stringify(existingRegistros));
  ...
  // Función para limpiar localStorage antes de guardar
  function cleanupLocalStorage() {
    try {
      ...
      if (validRequests.length !== recoveryRequests.length) {
        localStorage.setItem('passwordRecoveryRequests', JSON.stringify(validRequests));
        console.log('🧹 Limpiados tokens de recuperación expirados');
      }
      
      // Limpiar instantes viejos (más de 48 horas)
      const now = new Date().getTime();
      const keys = Object.keys(localStorage).filter(key => key.startsWith('instantes_'));
      
      keys.forEach(key => {
        try {
          const instantes = JSON.parse(localStorage.getItem(key) || '[]');
          const validInstantes = instantes.filter(i => {
            const createdAt = new Date(i.createdAt).getTime();
            return (now - createdAt) < (48 * 60 * 60 * 1000); // 48 horas
          });
          
          if (validInstantes.length !== instantes.length) {
            localStorage.setItem(key, JSON.stringify(validInstantes));
            console.log(`🧹 Limpiados instantes viejos de ${key}`);
          }
        } catch (e) {
          console.warn('Error limpiando instantes:', e);
        }
      });
  ...
  function setupAutoUsername() {
    ...
    displayNameInput.addEventListener('blur', () => {
      ...
      if (!usernameInput.value.trim() && displayValue.length >= 2) {
        const generated = generateUsername(displayValue);
  ...
  function loadPlansFromStorage() {
    ...
    console.log('📋 Registro - Cargando planes desde localStorage:', stored ? 'encontrado' : 'no encontrado');
  ...
  function updatePlanPriceDisplay(planId, days) {
    const plans = JSON.parse(localStorage.getItem('luxuryPlans') || '{}');
  ...
  function applyPlanRestrictions() {
    ...
    // Get plan restrictions from localStorage (configured by admin)
    const plans = JSON.parse(localStorage.getItem('luxuryPlans') || '{}');
  ...
  // Load payment configuration from localStorage
  function loadPaymentConfig() {
    // Load bank data
    const bankDataStored = localStorage.getItem('luxuryBankData');
    if (bankDataStored) {
      const bankData = JSON.parse(bankDataStored);
    ...
    const paymentOptionsStored = localStorage.getItem('luxuryPaymentOptions');
    if (paymentOptionsStored) {
      const options = JSON.parse(paymentOptionsStored);
  ...
  function updateAmountToTransfer(plan, duration = 30) {
  ...
  function setupPayLaterCheckbox() {
    ...
    payLaterCheckbox.addEventListener('change', () => {
  ...
  // Load interview time slots from localStorage (set by admin)
  function loadInterviewTimeSlots(timeSelect) {
    if (!timeSelect) return;
  ...
  function applyDiscountCode() {
    ...
    const storedCodes = localStorage.getItem('luxuryDiscountCodes');
    if (!storedCodes) {
